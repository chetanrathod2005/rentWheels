...
$stmt=$conn->prepare("SELECT book_to FROM fw_booking WHERE id=?");
...
$remark=$_POST['remark'];
$stmt=$conn->prepare("UPDATE fw_booking set booking_status=?,remark=? WHERE id=?");
...
$booking_id=$_POST["booking_id"];
$stmt=$conn->prepare("SELECT * FROM fw_booking WHERE id=?");
$stmt->bind_param("i",$booking_id);
$stmt->execute();
$result=$stmt->get_result();
$booking=$result->fetch_assoc();
$user_id=$booking['user_id'];
$msg = "Your vehicle request for " . $booking['car_name'] . " has been " . $booking['booking_status']."!";
