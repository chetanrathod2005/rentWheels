    ...
    $fileError= $_FILES['document']['error'];
     ...
     if(empty($bike_name) || empty($bike_rent) || empty($license) ||
    ...
    (user_id,bike_name,bike_rent,
    license,book_from,book_to,document,book_time)
    VALUES (?,?,?,?,?,?,?,?)
    ");
    $stmt->bind_param("isisssss",$user_id,$bike_name,$bike_rent,
    ...
    // send notification to admin
        ...
        $msg = "Request is received for " . 
        $booking['bike_name'] . " from " . 
        $booking['name'] . " for Date " . 
        $booking['book_from'] . " to " . 
        $booking['book_to'];
    date_default_timezone_set("Asia/Kolkata");
